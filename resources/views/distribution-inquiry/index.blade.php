...
<section class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-6">
                <div class="d-flex">
                    <h1>Inquiries for Distribution</h1>
                </div>
            </div>
            <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                    <li class="breadcrumb-item"><a href="{{ route('backend.dashboard') }}">Dashboard</a></li>
                    <li class="breadcrumb-item active">Distribution Inquiries</li>
                </ol>
            </div>
        </div>
    </div><!-- /.container-fluid -->
...
<div class="container-fluid pb-5">
    <div class="card shadow-sm">
        <div class="card-body p-0">
            @if(count($distributionInquiries))
            <table class="table">
                ...
                @foreach ($distributionInquiries as $inquiry)
                <tr>
                    <td>{{ $inquiry->name }}
                        @if(!$inquiry->viewed())
            ...
            @if($distributionInquiries->hasPages())
            <div class="d-flex py-2 px-4">
                <div>
                    Showing results from {{ $distributionInquiries->firstItem() }} to {{ $distributionInquiries->lastItem() }} of {{ $distributionInquiries->total() }}
                </div>
                <div class="ml-auto">
                    {{ $distributionInquiries->links() }}
                </div>
            </div>
            @endif
        </div>
...
<script>
    $(document).ready(function() {
        $('.inquiry-detail-modal').on('shown.bs.modal', function(e) {
            ...
            let apiUrl = "{{ route('backend.distribution-inquiry.mark-read') }}";
