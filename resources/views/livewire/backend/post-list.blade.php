            ...
            <form wire:submit.prevent="search" class="form">
                <div class="form-row align-items-center">
                    ...
                    <div class="col-auto form-group ">
                        ...
                        <select wire:model.defer="filter.post_category_id" class="custom-select">
        ...
        <x-box class="border-0 rounded shadow-sm">
            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>
                            <a class="" href="{{ route('backend.posts.edit', $post) }}">{{ $post->title }}</a>
                            <div>
                                @if ($post->trashed())
                        ...
                        <td><span class="badge badge-light font-weight-light">{{ $post->category->name }}</span></td>
                        <td>
                            <div class="badge {{ $post->isPublished() ? 'badge-success' : 'badge-danger' }} font-weight-light">
                        ...
                        <td class="text-right">
                            <div>
                                ...
                                <div class="dropdown-menu">
                                    ...
                                    @endif
                                    <a class="dropdown-item" href="{{ route('backend.posts.edit', $post) }}">Edit</a>
                                    @if($post->trashed())
                                    <form action="{{ route('backend.posts.restore', $post) }}" method="POST" class="form-inline d-inline">
                                        @csrf
                                        @method('patch')
                                        <button class="dropdown-item" type="submit">Restore</button>
                                    </form>
                                    <form action="{{ route('backend.posts.forceDelete', $post) }}" onsubmit="return confirm('This will permanantly delete post. Are you sure to delete?')" method="POST" class="form-inline d-inline">
                                        @csrf
                                        @method('delete')
                                        <button class="dropdown-item" type="submit">Delete</button>
                                    </form>
                                    @else
                                    <form action="{{ route('backend.posts.destroy', $post) }}" onsubmit="return confirm('Are you sure to delete?')" method="POST" class="form-inline d-inline">
                                        @csrf
                                        @method('delete')
